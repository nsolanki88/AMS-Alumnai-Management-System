...
export default function AlumniCalendar() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Mentorship & Workshop Calendar</h1>
        <p className="text-xs text-slate-500 mt-1">
          Your confirmed upcoming speaking sessions, laboratory workshops, and mentorship calls.
        </p>
      </div>

      {loading ? (
        <div className="text-center py-20 text-slate-400 text-xs">Loading calendar...</div>
      ) : sessions.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          <Calendar className="w-10 h-10 text-slate-300 mx-auto mb-2" />
          <p className="text-slate-600 font-semibold text-sm">No confirmed sessions on calendar</p>
          <p className="text-xs text-slate-400 mt-1">Accepted invitations will show up here.</p>
        </div>
      ) : (
            ...
            <div key={item.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-600 flex items-center gap-1 mt-1">
                  ...
                  Requester: <span className="font-semibold">{item.requester?.fullName}</span> ({item.requester?.role})
                </p>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                {item.meetingLink ? (
                  ...
                  >
                    <Video className="w-3.5 h-3.5" /> Launch Session
                  </a>
                ) : (
                  <span className="text-xs text-slate-400">Meeting link active soon</span>
                )}
              </div>
            </div>
          ))}
